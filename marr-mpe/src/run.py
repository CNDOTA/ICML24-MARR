...
def run(_run, _config, _log, pymongo_client=None):
    ...
    th.set_num_threads(4)  # restrict the number of used cpus for single instance
        ...
        if getattr(args, "runner_scope", "episodic") == "episodic":
            ...
            if buffer.can_sample(args.batch_size) and (buffer.episodes_in_buffer > getattr(args, "buffer_warmup", 0)):
                if args.replay_ratio > 1:
                    # speed up the training by sampling at once
                    episode_sample = buffer.sample_parallel(args.batch_size * args.replay_ratio)
                    ...
                    for i in range(args.replay_ratio):
                        learner.train(episode_sample[i * args.batch_size: (i + 1) * args.batch_size], runner.t_env, episode)
                elif args.replay_ratio == 1:
                ...
                else:  # if replay ratio == 0.5, agents will be updated every 2 episodes
                    if episode % (1 // args.replay_ratio) == 0:
                        episode_sample = buffer.sample(args.batch_size)
                        max_ep_t = episode_sample.max_t_filled()
                        episode_sample = episode_sample[:, :max_ep_t]
                        if episode_sample.device != args.device:
